...
for (const error of ['live_data_connector_not_configured', 'hard_reasoning_not_verified', 'chat_provider_rate_limited', 'message_too_large', 'image_generation_not_enabled', 'image_generation_unavailable', 'image_generation_failed', 'attachment_invalid', 'attachment_unsupported', 'attachment_too_large', 'attachment_conversion_failed', 'attachment_conversion_unavailable']) {
  ...
  if (!error.startsWith('attachment_') && !error.startsWith('image_generation_')) assert.match(outcome.answer, /kept below/);
}
...
const generated = chatOutcome(ok, { ok: true, status: 'completed', answer: 'Generated image.', truth: 'generated-image', image: { mimeType: 'image/jpeg', data: 'ZmFrZQ==' } });
assert.equal(generated.complete, true);
assert.equal(generated.label, 'Image generated');
assert.equal(generated.images.length, 1);
assert.match(generated.note, /generated by PI/i);
