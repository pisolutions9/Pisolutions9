...
const BLOCKERS = {
  ...
  image_generation_not_enabled: { answer: 'PI image generation is prepared but not enabled on this deployment yet.', label: 'Image generation not enabled' },
  image_generation_unavailable: { answer: 'PI image generation is temporarily unavailable.', label: 'Image generation unavailable' },
  image_generation_failed: { answer: 'PI could not generate that image reliably. No completed image is being claimed.', label: 'Image generation failed' },
...
export function chatOutcome(response, body) {
  ...
  const notes = {
    ...
    'generated-image': 'Image generated by PI’s configured image model.',
  ...
  const completedImage = body.truth === 'generated-image' && body.status === 'completed' && body.image && typeof body.image.data === 'string';
  ...
  return {
    ...
    label: completedImage ? 'Image generated' : completedArtifact ? 'File verified' : body.truth === 'needs-input' ? 'More detail needed' : limited ? 'Limited reply' : 'Reply received',
    ...
    images: completedImage && /^image\/(jpeg|png|webp)$/.test(body.image.mimeType || '') && /^[A-Za-z0-9+/]+={0,2}$/.test(body.image.data) ? [body.image] : [],
  };
}
