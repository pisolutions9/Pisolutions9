...
async function ask(message, history=[], { requireModel = true } = {}) {
  // Retry transient transport/provider recovery, but never count recovery as model success.
  let lastResult=null;
  let lastResponse=null;
  for(let attempt=0;attempt<3;attempt++){
    try{
      lastResponse=await fetch(endpoint,{method:'POST',headers:{'content-type':'application/json',origin:'https://pisolutions9.github.io'},body:JSON.stringify({message,history}),signal:AbortSignal.timeout(65000)});
      assert.equal(lastResponse.status,200);
      lastResult=await lastResponse.json();
      assert.equal(lastResult.ok,true);
      const recovered=lastResult.truth==='deterministic'||lastResult.source==='pi-chat-deterministic-recovery';
      if(!requireModel||!recovered)return lastResult;
