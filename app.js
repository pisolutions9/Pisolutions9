...
function renderGeneratedImages(images, card) {
  if (!Array.isArray(images)) return;
  for (const image of images.slice(0, 1)) {
    if (!image || !/^image\/(jpeg|png|webp)$/.test(image.mimeType || '') || typeof image.data !== 'string' || !/^[A-Za-z0-9+/]+={0,2}$/.test(image.data)) continue;
    const src = `data:${image.mimeType};base64,${image.data}`;
    const element = document.createElement('img'); element.src = src; element.alt = 'Image generated by PI'; element.className = 'generated-image'; card.append(element);
    const link = document.createElement('a'); link.href = src; link.download = 'pi-generated-image.jpg'; link.textContent = 'Save image'; link.className = 'download'; card.append(link);
  }
}
...
async function runCustomerChat(text, attachment = null) {
  ...
  try {
    ...
    renderGeneratedImages(outcome.images, card);
