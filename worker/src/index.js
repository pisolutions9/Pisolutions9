...
const IMAGE_INTENT = /^(?:please\s+)?(?:create|generate|make|draw|design|render)\b[\s\S]{0,240}\b(?:image|picture|photo|illustration|poster|wallpaper|logo|artwork)\b/i;
function imagePrompt(text=''){const value=String(text).trim();return IMAGE_INTENT.test(value)?value.slice(0,2048):null;}
async function generateImage(env,prompt){
  if(env.PI_IMAGE_GENERATION_ENABLED!=='true')throw new Error('image_generation_not_enabled');
  if(!env.AI||typeof env.AI.run!=='function')throw new Error('image_generation_unavailable');
  const model=env.PI_IMAGE_MODEL||IMAGE_MODEL;
  const result=await env.AI.run(model,{prompt,steps:4});
  if(!result||typeof result.image!=='string'||!result.image.trim())throw new Error('image_generation_failed');
  return {model,mimeType:'image/jpeg',data:result.image.trim()};
}
...
export default{async fetch(request,env){const url=new URL(request.url);const origin=request.headers.get('Origin')||'';if(url.pathname!=='/api/chat')return new Response('Not found',{status:404});if(origin&&origin!==ALLOWED_ORIGIN)return json({ok:false,error:'origin_not_allowed'},403,request);if(request.method==='OPTIONS')return preflight(request);if(request.method!=='POST')return json({ok:false,error:'method_not_allowed'},405,request);let payload;try{payload=await request.json();}catch{return json({ok:false,error:'invalid_json'},400,request);}const message=String(payload?.message||'').trim();if(!message)return json({ok:false,error:'message_required'},400,request);if(message.length>MAX_INPUT)return json({ok:false,error:'message_too_large'},413,request);let history=[];let attachment=null;let attachmentInfo=null;try{history=validateHistory(payload.history);attachment=validateAttachment(payload.attachment);if(!attachment){const prompt=imagePrompt(message);if(prompt){try{const image=await generateImage(env,prompt);return json({ok:true,status:'completed',answer:'Generated image.',truth:'generated-image',source:`pi-image-cloudflare:${image.model}`,image:{mimeType:image.mimeType,data:image.data}},200,request);}catch(error){const code=String(error?.message||error);return json({ok:false,status:'image_generation_blocked',error:code,answer:'PI image generation is not available on this deployment yet.',truth:'unknown'},503,request);}}const mission=inventoryMission(message);if(mission)return json(mission,200,request);}if(attachment)attachmentInfo=await attachmentContext(env,attachment);}catch(error){const code=String(error?.message||error);const status=code==='attachment_conversion_unavailable'||code==='attachment_conversion_failed'?503:400;return json({ok:false,error:code},status,request);}const effectiveMessage=withAttachment(message,attachmentInfo);if(requiresLiveEvidence(message)){if(env.OPENAI_API_KEY){const model=env.PI_WEB_MODEL||env.PI_CHAT_MODEL||'gpt-5';try{const response=await callProvider({apiKey:env.OPENAI_API_KEY,model,baseUrl:'https://api.openai.com/v1',message:effectiveMessage,history,useWebSearch:true,instructions:`${PI_INSTRUCTIONS} This request requires current information. Use web search, distinguish sourced facts from uncertainty, and do not claim freshness beyond the retrieved evidence.`});if(!response.ok){const failure=providerError(response);return json({ok:false,status:'live_evidence_required',error:failure.error,answer:'PI could not verify current information from the live research provider, so it will not guess.',truth:'unknown'},failure.status,request,rateLimitHeaders(response));}const body=await response.json();const answer=extractAnswer(body);if(!answer)return json({ok:false,status:'live_evidence_required',error:'empty_live_research_response',answer:'PI could not produce a sourced current answer, so it will not guess.',truth:'unknown'},503,request);return json({ok:true,status:'answered',answer,source:`pi-chat-web:${model}`,truth:'web-grounded-model-response',sources:extractSources(body)},200,request);}catch{return json({ok:false,status:'live_evidence_required',error:'live_research_provider_unavailable',answer:'PI could not reach the live research provider, so it will not guess.',truth:'unknown'},503,request);}}return json({ok:false,status:'live_evidence_required',error:'live_data_connector_not_configured',answer:'I need a live data source to answer that accurately. I will not guess or present model memory as current data.',truth:'unknown'},503,request);}const hardReasoning=requiresHardReasoning(effectiveMessage);
if(hardReasoning){
