...
function normalizeWeatherLocation(location=''){
  ...
  if(!match)return raw;
  const state=US_STATE_ABBREVIATIONS.get(match[2].toUpperCase());
  return state?`${match[1].trim()}, ${state}`:raw;
}
...
function weatherLocationQuery(message=''){
  ...
  if(prefix?.[1]){
    ...
    const looksLikeQuestionLead=/^(?:what|whats|what's|how|tell|give|is|are|can|could|would)\b/i.test(cleaned);
...
async function directWeatherAnswer(message='',history=[]){
  const location=normalizeWeatherLocation(weatherLocationQuery(message));
  if(!location){
    if(Array.isArray(history)&&history.length&&/\b(weather|forecast|temperature|rain|snow|humidity|wind)\b/i.test(String(message))){
      return {
        ok:true,
