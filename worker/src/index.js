...
async function callProvider({apiKey,model,baseUrl,message,history=[],timeoutMs=PROVIDER_TIMEOUT_MS}){const controller=new AbortController();const boundedTimeout=Math.max(1,Math.min(PROVIDER_TIMEOUT_MS,timeoutMs));const timer=setTimeout(()=>controller.abort(),boundedTimeout);try{return await fetch(`${baseUrl.replace(/\/$/,'')}/responses`,{method:'POST',headers:{authorization:`Bearer ${apiKey}`,'content-type':'application/json'},body:JSON.stringify({model,store:false,max_output_tokens:MAX_OUTPUT_TOKENS,instructions:PI_INSTRUCTIONS,input:[...history,{role:'user',content:message}]}),signal:controller.signal});}finally{clearTimeout(timer);}}
